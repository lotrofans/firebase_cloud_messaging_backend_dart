...
class FirebaseCloudMessagingServer {
  ...
  Future<ServerResult> _send(Send sendObject) async {
    ...
    var response = await http.post(Uri.parse(url),
        headers: {"Content-Type": "application/json", "Authorization": "Bearer ${accessCredentials!.accessToken.data}"},
        body: json.encode(sendObject.toJson()));
    var successful = response.statusCode == 200;
    print("successful: $successful");
    var serverResult =
        ServerResult(successful, response.statusCode, successful ? Message.fromJson(json.decode(response.body)) : Message(), response.reasonPhrase);
    print(serverResult);
    return serverResult;
  }
}
...
class ServerResult {
  ...
  ServerResult(this.successful, this.statusCode, this.messageSent, this.errorPhrase);
